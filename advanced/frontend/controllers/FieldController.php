...
class FieldController extends Controller
{
     ...
     */
    public function actionField_add(){
      //判断是不是post传值
      if(\Yii::$app->request->post()){
        ...
        if($result){
          $this->redirect('?r=industry/field_list');
        }else{
          die("<script>alert('网络错误，添加失败');location.href='?r=industry/field_add'</script>");
        }
        // echo "post";die;
      }else{
        ...
        return $this->render('field_add.html',['category'=>$category]);
      }
      
    }
    public function actionField_list(){
         $arr = PosCategory::find();
         ...
         return $this->render('field_list.html',['category'=>$cate_info,'pages'=>$pages]);
  ...
  public function actionPosition_del(){
    ...
    if(empty($str)){
        ...
        if($res){
          ...
          echo "no";
          die("<script>alert('对不起，网络错误');location.href='?r=industry/field_list'</script>");
        }
    }else{
        echo 0;
